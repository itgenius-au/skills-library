---
name: finish-branch
description: An override of superpowers:finishing-a-development-branch. Use when implementation is complete, all tests pass, and you need to decide how to integrate the work — guides completion of development work by presenting structured options for merge, PR, or cleanup. Fixes the cwd-vanishes-inside-worktree bug and adds a clean-tree gate before options are presented (pending upstream at obra/superpowers#1223).
---

# Finish Branch (local override)
...
Cannot proceed — finish-branch only integrates *committed* work. Commit or stash first (squash your wip commits into one clean commit with `git reset --soft <base> && git commit`, session-wrap, or plain `git add` + `git commit`), then re-run.
```

Stop. Don't proceed. The user must decide what belongs on the branch before finishing it — this skill will not stage changes on their behalf.
...
- **Fix:** In Step 1, refuse if `git status --porcelain` is non-empty. Point user to squash their wip commits into one clean commit (`git reset --soft <base> && git commit`), session-wrap, or `git add` + `git commit` first.
