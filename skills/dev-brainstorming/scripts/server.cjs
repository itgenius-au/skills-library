...
// Branding is a plain local label (see brandMarkup) - no external brand image,
// version reader, telemetry toggle, or upstream link.
...
function brandMarkup() {
  // A plain local label, no external image or link.
...
function startServer() {
  ...
  function onListen() {
    ...
    console.log(info);
    // server-info embeds the key, so keep it owner-only.
